...
export default function PlaceMapPage() {
        ...
        >
          <PlaceInfoContents>
            <PlaceInfoRow>
              ...
              {placeHistoryList.length > 0 && (
                <Rating
                  readOnly={true}
                  value={placeHistoryList[0].rating}
                  max={placeHistoryList[0].rating}
                  size="medium"
                  variant="iconOnly"
          ...
          myPlaceList.map((place) => {
            return (
              <Marker
                key={place.id}
                label={{
                  text: `[${categoryMap.get(place.category)}] ${place.name}`,
                  color: "#000",
                  fontSize: "20px",
                  fontWeight: "bold",
                }}
  ...
  const buttonView = () => {
    return (
      <ButtonArea>
        <Button size="small" onClick={onCurrentLocationClick}>
