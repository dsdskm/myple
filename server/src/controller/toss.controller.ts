...
export const getUserInfo = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `getUserInfo authorizationCode ${authorizationCode} referrer ${referrer} tossToken ${JSON.stringify(tossToken)}`,
    );
    if (tossToken) {
...
export const requestTossOrders = async (req: Request, res: Response) => {
  try {
    const { userKey, orderId } = req.body;
    const result = await tossService.requsetTossOrders(userKey, orderId);
