...
export const requestTossAccessToken = async (authorizationCode: string, referrer: string): Promise<TossToken> => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          if (!parsed.success) {
            return reject(new Error(`Toss API error: ${data}`));
          }
...
function calculateScore(result: any): number {
  ...
  if (result.address_components) {
    const requiredComponents = ["street_number", "route", "locality", "administrative_area_level_1", "country"];
